...
export async function GET(request: NextRequest) {
  try {
    ...
    const designs = await SavedDesign.find({ userEmail: email })
      .sort({ createdAt: -1 })
      .lean();
...
export async function POST(request: Request) {
  try {
    ...
    const { userEmail, name, preview, config } = body;
...
// DELETE /api/users/designs — Delete a saved design
export async function DELETE(request: Request) {
  try {
    ...
    if (!designId) {
      return Response.json({ success: false, error: "Design ID is required" }, { status: 400 });
    }
